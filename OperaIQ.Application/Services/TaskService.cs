...
namespace OperaIQ.Application.Services
{
    public class TaskService : ITaskService
    {
        ...
        public async Task<IEnumerable<TaskDto>?> GetTasksByProjectAsync(Guid projectId, Guid userId, bool canSeeAll)
        {
            // Nhân viên thường chỉ xem được task của dự án mình tham gia.
            if (!canSeeAll)
            {
                var memberships = await _memberRepo.FindAsync(
                    m => m.ProjectId == projectId && m.UserId == userId);
                if (!memberships.Any()) return null;
            }

            // Include Project + AssignedTo để map ProjectName / AssignedToName.
            var tasks = await _taskRepo.FindAsync(
                t => t.ProjectId == projectId, t => t.Project, t => t.AssignedTo);
        ...
        {
            var tasks = await _taskRepo.FindAsync(
                t => t.AssignedToId == userId, t => t.Project, t => t.AssignedTo);
            return _mapper.Map<IEnumerable<TaskDto>>(tasks);
        }

        public async Task<TaskDto?> GetTaskByIdAsync(Guid id)
        {
            var task = await _taskRepo.GetByIdAsync(id, t => t.Project, t => t.AssignedTo);
        ...
        public async Task<Result<Guid>> CreateTaskAsync(CreateTaskDto dto)
        {
            ...
            // Tính toán lại tiến độ dự án khi thêm task mới
        ...
        public async Task<Result> UpdateTaskStatusAsync(Guid id, TaskStatus status)
        {
            ...
            // Tự động cập nhật ProgressPercent theo TaskStatus
        ...
        {
            var project = await _projectRepo.GetByIdAsync(projectId, p => p.Tasks);
            if (project != null)
            {
                var projectTasks = project.Tasks.Where(t => t.Status != TaskStatus.Cancelled).ToList();
                if (projectTasks.Any())
                {
                    int totalEstimatedHours = projectTasks.Sum(t => t.EstimatedHours);
                    if (totalEstimatedHours > 0)
                    {
                        double weightedProgress = projectTasks.Sum(t => (double)t.ProgressPercent * t.EstimatedHours);
                        project.ProgressPercent = (int)Math.Round(weightedProgress / totalEstimatedHours);
                    }
                    else
                    {
                        // Fallback to average progress
                        project.ProgressPercent = (int)Math.Round(projectTasks.Average(t => t.ProgressPercent));
                    }
                }
                else
                {
                ...
                // Tự động cập nhật trạng thái dự án nếu toàn bộ công việc đã hoàn thành
                if (projectTasks.Any() && projectTasks.All(t => t.Status == TaskStatus.Done))
                {
                    project.Status = ProjectStatus.Completed;
                }
                else if (project.Status == ProjectStatus.Completed && projectTasks.Any(t => t.Status != TaskStatus.Done))
                {
